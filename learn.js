...
// let alarmsHistoryNum =  parseInt(prompt('Enter number of last alarms data to show: '));
  ...
  function callbackHistory(error, response, body) {
...
function invokeHistoryServer() {
    request(optionsHistoryAlarms, callbackHistory);
}

 function invokeAlarmServer() {
    new Promise(((resolve, reject) => {
        setInterval(() => {
            request(optionsServer, callback);
        }, 2000)
...
new Promise(resolve => {
    console.log('starting alarms server! you will get a message if attack will happen.')
})
...
module.exports = {invokeHistoryServer, invokeAlarmServer}
